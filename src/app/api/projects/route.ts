...
export async function GET(req: Request) {
  ...
  if (url.pathname.endsWith('/export')) {
    ...
    for (const ann of annotations) {
      ...
      const _unused = { _id, convProjectId, convCreatedAt, convUpdatedAt, convTurns };
      exportObjects.push({
        type: 'conversation',
...
export async function DELETE(req: Request) {
  ...
  const client = await clientPromise;
  const db = client.db(process.env.DB_NAME);
  const result = await db.collection('projects').deleteOne({ _id: typeof projectId === 'string' ? new (await import('mongodb')).ObjectId(projectId) : projectId });
  if (result.deletedCount === 1) {
    return NextResponse.json({ ok: true });
  } else {
    return NextResponse.json({ error: 'Project not found or not deleted' }, { status: 404 });
  }
} 